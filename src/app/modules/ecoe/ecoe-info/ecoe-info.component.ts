...
export class EcoeInfoComponent implements OnInit {
  ...
  restoreEcoe() {
    ...
    const resource = "ecoes/archive/" + this.ecoeId + "/restore";
  ...
  submitECOENameForm() {
    ...
    new ECOE(this.ecoe)
      .update({ name: this.ecoe_name_form.get("ecoe_name_2edit").value })
      .then((response) => {
        this.message.success(this.translate.instant("OK_REQUEST_CONTENT"), {
          nzDuration: 5000,
        });
        this.ecoe = response;
        this.ecoeName = this.ecoe.name;
